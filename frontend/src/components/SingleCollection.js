...
const SingleCollection = () => {
  ...
  // Memoize sortedAndFilteredProducts
  const visibleProducts = useMemo(() => sortedAndFilteredProducts, [sortedAndFilteredProducts]);
  ...
  // Memoize CollectionProductItem
  const CollectionProductItem = React.memo(({ product, index, inView, navigate, viewMode }) => (
    <motion.div
      key={product._id}
            ...
            <div className={`gap-4 sm:gap-6 ${viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4' : 'flex flex-col'}`}>
              {visibleProducts.length === 0 ? (
                ...
                visibleProducts.map((product, index) => (
                  <CollectionProductItem key={product._id} product={product} index={index} navigate={navigate} viewMode={viewMode} />
                ))
              )}
            </div>
          </div>
